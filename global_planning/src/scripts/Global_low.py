...
import time
import csv
import yaml  #安装时输入 \pip install pyyaml
import os
import rospy
...
f.close()
robot_range = programconfig.get("robot_size_3")
safe_range=programconfig.get("safe_distance_3")
safe_distance=robot_range+safe_range
angle_jiange=programconfig.get("angle_jiange_3")
safe_distance=robot_range+safe_range
...
init_path = Path()
def sendAstarPath_3(path_x,path_y,path_z,angle_x,angle_y,angle_z,angle_w):
        # rospy.init_node("globel_path_planning",anonymous=True)
        AstarPath = rospy.Publisher("AstarPath_Low",Path,queue_size=15)
...
#         if IMG_arr[i][j] == 76: #红色灰度值
#             IMG_arr1[i][j] = 255 #白色灰度值


IMG_binary = np.where(220< IMG_arr, 255, 0)#原始图像中的灰色和黑色区域为障碍物区域，其他区域为可行区域，可行区域的灰度值置为255，障碍物区域的灰度值置为0  灰色区域205

def add_obstacle(x,y):
    obb_x=[]
    obb_y=[]
    for i in range(-(safe_distance),(safe_distance)+1,safe_distance):
        for j in range(-(safe_distance), (safe_distance)+1, safe_distance):
            m = [x + i, y + j]
...
path_x_reverse=[]
path_y_reverse=[]
path_x_real_reverse_3=[]
path_y_real_reverse_3=[]
path_z_real_3=[]
path_yaw=[]
...
with io.open(fname, 'r', encoding='utf-8') as f:
    for line in f.readlines():
        if line.isspace():         #如果是空行则跳过该行
            ...
            # if IMG_binary[p_real_y][p_real_x] != 0 and IMG_binary[p_real_y][p_real_x] != 2:
...
for i in range(len(xiaodu)):
    ...
    if i<len(xiaodu)-1:
        ...
        for i in range(len(path_all)):
            ...
            path_x_real_reverse_3.append((path_x[len(path_all) - 1 - i]/(p_w/ori_p_w)+int(origion_x/map_resolution))*map_resolution)  #再将消毒路径像素点的坐标转化为二维物理坐标
            path_y_real_reverse_3.append((ori_p_h-path_y[len(path_all) - 1 - i]/(p_h /ori_p_h)+int(origion_y/map_resolution))*map_resolution)
...
for i in range(len(path_x_real_reverse_3)):
    if i<len(path_x_real_reverse_3)-angle_jiange:
        path_a=(path_x_real_reverse_3[i],path_y_real_reverse_3[i])
        path_b=(path_x_real_reverse_3[i+angle_jiange],path_y_real_reverse_3[i+angle_jiange])
        path_yaw.append(rad_ptA_ptB(path_a,path_b))
    if len(path_x_real_reverse_3)-angle_jiange<=i<len(path_x_real_reverse_3)-1:
        path_a=(path_x_real_reverse_3[i-9],path_y_real_reverse_3[i-9])
        path_b=(path_x_real_reverse_3[i+1],path_y_real_reverse_3[i+1])
        path_yaw.append(rad_ptA_ptB(path_a,path_b))
#以下为消毒路径的偏航角等三维物理信息
qq_3=[]
qxx_3=[]
...
for i in range(len(path_yaw)):
    ...
    qq_3.append(quaternion[0])
    qxx_3.append(quaternion[1])
    qyy_3.append(quaternion[2])
    qzz_3.append(quaternion[3])
    # qww.append(quaternion[4])


sendAstarPath_3(path_x_real_reverse_3,path_y_real_reverse_3,path_z_real_3,qq_3,qxx_3,qyy_3,qzz_3)
csv_save(file,path_x_real_reverse_3,path_y_real_reverse_3,path_z_real_3,qq_3,qxx_3,qyy_3,qzz_3)
