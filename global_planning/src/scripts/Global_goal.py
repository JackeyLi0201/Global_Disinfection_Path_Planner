...
f.close()
robot_range = programconfig.get("robot_size_1")
safe_range=programconfig.get("safe_distance_1")
safe_distance=robot_range+safe_range
angle_jiange=programconfig.get("angle_jiange_1")
...
init_path = Path()
def sendAstarPath(path_x,path_y,path_z,angle_x,angle_y,angle_z,angle_w):
...
def min_xiaodu(na):
    ...
    for i in range(len(na)):
        ...
        if dis<=dis_initial:
            ...
            min_index=i
    return min_index
with open(mapyaml, "r") as f: #office_en_large jackal_race
...
# IMG = Image.open("//home//wang//桌面//4.png")
#        img = img.resize((100,100))  ### resize图片尺寸
IMG_gray = IMG.convert('L')  # 地图灰度化 彩色图像转为深度图
IMG_arr = np.array(IMG_gray)
ori_p_h=IMG_arr.shape[0] #原始地图的高，也就是y轴方向的像素点个数
ori_p_w=IMG_arr.shape[1] #原始地图的宽，也就是x轴向的像素点个数
...
with io.open(fname, 'r', encoding='utf-8') as f:
    for line in f.readlines():
        if line.isspace():         #如果是空行则跳过该行
            ...
            p_real_x = int((p_x - o_x)) #消毒点的像素点坐标y
            p_real_y=int((ori_p_h-(p_y-o_y)))  #消毒点的像素点坐标x
...
for i in range(len(xiaodu)):
    ...
    if i<len(xiaodu)-1:
        ...
        for i in range(len(path_all)):
            ...
            path_x_real_reverse.append((path_x[len(path_all) - 1 - i]+int(origion_x/map_resolution))*map_resolution)  #再将消毒路径像素点的坐标转化为二维物理坐标
            path_y_real_reverse.append((ori_p_h-path_y[len(path_all) - 1 - i]+int(origion_y/map_resolution))*map_resolution)
...
for i in range(len(path_x_real_reverse)):
    if i<len(path_x_real_reverse)-angle_jiange:
        path_a=(path_x_real_reverse[i],path_y_real_reverse[i])
        path_b=(path_x_real_reverse[i+angle_jiange],path_y_real_reverse[i+angle_jiange])
        path_yaw.append(rad_ptA_ptB(path_a,path_b))
    if len(path_x_real_reverse)-angle_jiange<=i<len(path_x_real_reverse)-1:
        path_a=(path_x_real_reverse[i-9],path_y_real_reverse[i-9])
        path_b=(path_x_real_reverse[i+1],path_y_real_reverse[i+1])
        path_yaw.append(rad_ptA_ptB(path_a,path_b))
...
for i in range(len(path_yaw)):
    ...
    qq.append(quaternion[0])
    qxx.append(quaternion[1])
    qyy.append(quaternion[2])
    qzz.append(quaternion[3])
    # qww.append(quaternion[4])


sendAstarPath(path_x_real_reverse,path_y_real_reverse,path_z_real,qq,qxx,qyy,qzz)

csv_save(file,path_x_real_reverse,path_y_real_reverse,path_z_real,qq,qxx,qyy,qzz)
